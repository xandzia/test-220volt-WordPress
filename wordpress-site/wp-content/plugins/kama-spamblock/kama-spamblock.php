...
class Kama_Spamblock {
	...
	function spamblock( $commentdata ){
		...
		if( $com_type == 'trackback' || $com_type == 'pingback' ){
	...
	static function make_nonce( $key ){
		if( preg_match('/^[a-f0-9]{32}$/', $key) ) return $key; // уже md5
		return md5( $key );
	}
	...
	## default_options
	function def_opt(){
		// никаких динамических опций тут!!!
		return array(
	...
	static function sanitize_opt( $opts ){
		foreach( $opts as $key => & $val ){
			if( $key === 'sibmit_button_id' )
				$val = sanitize_html_class( $val );
			elseif( $key === 'unique_code' )
